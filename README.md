...
To update the database use this command from the Web project folder (replace `CleanArchBlog_crud` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../CleanArchBlog_crud.Infrastructure/CleanArchBlog_crud.Infrastructure.csproj -s CleanArchBlog_crud.Web.csproj
```
